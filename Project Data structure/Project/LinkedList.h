...
public:
	...
	*/
	...
	Pickable* RetrieveVisitorID(Pickable * inputVisitor) //to retrieve visitor needed by its ID
	{
		Node<Pickable*>* temp = Head;
		while (temp)
		{
			if (*(temp->getItem()) == *inputVisitor) //if found
			{
				Pickable* retrievedVisitor = temp->getItem(); //set retrieved with found element
				DeleteNode(temp->getItem()); //deleting visitor from list
				return retrievedVisitor; //return found visitor
			}
			temp = temp->getNext();
		}
		return nullptr;
